...
<body>
	...
	<div class="container">
		...
		<c:if test="${dto.writer eq id }">
			<a class="btn btn-outline-warning btn-sm"
				href="private/updateform.do?num=${dto.num }"> <svg width="1em"
			...
			<a class="btn btn-outline-danger btn-sm"
				href="javascript:deleteConfirm();"> <svg width="1em"
		...
		<div class="row border border-primary mx-0">
		
			<div class="col p-0">
				<img src="${pageContext.request.contextPath}${dto.image}" class="w-100" />
			</div>
			
			<div class="col p-0">
				<p class="pt-5 mt-5">상품번호: [${dto.num }]</p>
				<p>작 성 자: ${dto.writer }</p>
				<p>상 품 명: ${dto.name }</p>
				<p>판 매 가: <fmt:formatNumber type="currency" value="${dto.price }" currencySymbol=""/>원</p>
				
				<p>등 록 일: ${dto.regdate }</p>
			</div>
			
		</div>
		<div class="contents my-3">${dto.content }</div>
					...
					<c:choose>
						...
						<c:otherwise>
							<li id="comment${tmp.num }"
									...
									<dt>
										<c:choose>
											<c:when test="${empty tmp.profile }">
												<svg class="profile-image" width="1em" height="1em"
													viewBox="0 0 16 16" class="bi bi-person-fill"
													fill="currentColor" xmlns="http://www.w3.org/2000/svg">
					  							<path fill-rule="evenodd"
										...
										| <a data-num="${tmp.num }" href="javascript:"
												class="comment-update-link">수정</a>
										| <a data-num="${tmp.num }" href="javascript:"
												class="comment-delete-link">삭제</a>
										</c:if>
									</dt>
									<dd>
										<pre>${tmp.content }</pre>
								...
								</form> <!-- 로그인된 아이디와 댓글의 작성자가 같으면 수정 폼 출력 --> <c:if
			...
			<ul class="pagination pagination-sm">
				<c:if test="${startPageNum ne 1 }">
					<li class="page-item"><a class="page-link"
						href="detail.do?num=${dto.num }&pageNum=${startPageNum-1 }">Prev</a></li>
				</c:if>
				<c:forEach var="i" begin="${startPageNum }" end="${endPageNum }">
					<c:choose>
						<c:when test="${i eq pageNum }">
							<li class="page-item active"><a class="page-link"
								href="detail.do?num=${dto.num }&pageNum=${i }">${i }</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="detail.do?num=${dto.num }&pageNum=${i }">${i }</a></li>
				...
				<c:if test="${endPageNum lt totalPageCount }">
					<li class="page-item"><a class="page-link"
						href="detail.do?num=${dto.num }&pageNum=${endPageNum+1 }">Next</a></li>
				</c:if>
			</ul>
		</div>
		<!-- 원글에 댓글을 작성하는 form -->
	...
	$(document).on("click",".comment-delete-link", function(){
		...
		if(isDelete){
			...
			"/product/private/comment_delete.do?num="+num+"&ref_group=${dto.num}";
		}
	});
	...
	$(document).on("click",".reply-link", function(){
		...
		if(isLogin == false){
			...
			location.href="${pageContext.request.contextPath }/users/loginform.do?"+
					"url=${pageContext.request.contextPath }/product/detail.do?num=${dto.num}";
		}
